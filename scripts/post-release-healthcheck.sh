...
BASE_URL="${1:-${LZKB_RELEASE_URL:-}}"
TIMEOUT_SECONDS="${LZKB_HEALTHCHECK_TIMEOUT_SECONDS:-180}"
INTERVAL_SECONDS="${LZKB_HEALTHCHECK_INTERVAL_SECONDS:-5}"

if [[ -z "${BASE_URL}" ]]; then
  echo "Usage: $0 https://lzkb.example.com" >&2
  exit 2
fi
